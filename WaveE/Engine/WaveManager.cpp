...
namespace WaveE
{
	...
	WaveManager::WaveManager(const WaveEDescriptor& rDescriptor)
	{
		...
		m_samplerHeap.Init(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, D3D12_DESCRIPTOR_HEAP_FLAG_NONE, m_descriptorHeapCountSampler);
		...
		m_defaultInputLayout = { inputElementDescs, _countof(inputElementDescs) };
	}
	...
	{
		WAVEE_ASSERT_MESSAGE(IsCBV_SRV_UAVSlot(slot), "Invalid slot index for sampler!");
		BindResource(rb.allocation, slot);
	}

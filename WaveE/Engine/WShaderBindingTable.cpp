...
namespace WaveE
{
	...
	WShaderBindingTable::WShaderBindingTable(const WShaderBindingTableDescriptor& rDescriptor)
	{
		...
		shaderBindingTableBufferDesc.sizeBytes = m_rayGenRecordSize + m_missRecordSize * rDescriptor.missLocalRootArgs.size() + m_hitGroupRecordSize * rDescriptor.missLocalRootArgs.size();
		...
		// ------------------- Miss Shader Table -------------------
		{
			for (UINT i = 0; i < m_missCount; ++i)
			{
				...
				{
					m_shaderBindingTableBufferID.GetResource()->UploadData(rDescriptor.missLocalRootArgs.data(), rDescriptor.missLocalRootArgs.size(), currentOffset + shaderIdSize);
				}
		...
		// ------------------- Hit Group Shader Table -------------------
		{
			for (UINT i = 0; i < m_hitGroupCount; ++i)
			{
				...
				{
					m_shaderBindingTableBufferID.GetResource()->UploadData(rDescriptor.hitGroupLocalRootArgs.data(), rDescriptor.hitGroupLocalRootArgs.size(), currentOffset + shaderIdSize);
				}
